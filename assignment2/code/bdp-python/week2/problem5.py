        ...
        for event in events:
            ...
            if parse(date_hour) >= time_range[0] and parse(date_hour) <= time_range[1]:
                ...
                if date_hour in local_dict:
                    if country in local_dict[date_hour]:
                        local_dict[date_hour][country].add(url)  # sets allow only unique values
                    else:
                        local_dict[date_hour][country] = set([url])
                else:
                    local_dict[date_hour] = {country: set([url])}
...
if __name__ == '__main__':
    ...
    with multiprocessing.Manager() as manager:
        ...
        for file_dict in shared_results:
            for date_hour in file_dict.keys():
                if date_hour in final_dict:
                    for country in file_dict[date_hour]:
                        if country in final_dict[date_hour]:
                            final_dict[date_hour][country] |= file_dict[date_hour][country]
                        else:
                            final_dict[date_hour][country] = file_dict[date_hour][country]
                else:
        ...
        for query_params in queries:
            query = final_dict[f"{query_params[0]}:{query_params[1]}"][query_params[2]]
            print(f"{query_params[0]}, {query_params[1]}, {query_params[2]} : {len(query)}")
