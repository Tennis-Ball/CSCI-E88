...
# {"2022-09-04:07": {"url1": {user-01, user-02}, "url2": {...}, ...}, ...}
        ...
        for event in events:
            ...
            if date_hour in local_dict:
                if url in local_dict[date_hour]:
                    local_dict[date_hour][url].add(user)  # sets allow only unique values
                else:
                    local_dict[date_hour][url] = set([user])
            else:
                local_dict[date_hour] = {url: set([user])}
...
if __name__ == '__main__':
    ...
    with multiprocessing.Manager() as manager:
        ...
        # QUERY 1, PROBLEM 3.3.1
        for hour in ["07", "08", "09", "10", "11"]:
            date_hour = "2022-09-04:" + hour
            print(f"{date_hour}, {len(final_dict[date_hour])}")
        
        # QUERY 2, PROBLEM 3.3.2
        for url_id in range(1, 6):
            date_hour = "2022-09-06:13"
            url = f"http://example.com/?url=00{url_id}"
            print(f"{date_hour}, {url}, {len(final_dict[date_hour][url])}")
