...
def do_work(shared_results, lock, file_name):
    ...
    with open(file_name) as file_handle:
        ...
        for event in events:
            date_hour = event[1][:event[1].find("T")]  # parse datetime into date
            url = event[2]
            TTFB = float(event[8])
            ...
            if date_hour in local_dict:
                if url in local_dict[date_hour]:
                    local_dict[date_hour][url].append(TTFB)  # sets allow only unique values
                else:
                    local_dict[date_hour][url] = [TTFB]
            else:
                local_dict[date_hour] = {url: [TTFB]}
...
if __name__ == '__main__':
    ...
    with multiprocessing.Manager() as manager:
        ...
        for file_num, file_dict in enumerate(shared_results):
            for date in file_dict.keys():
                if date in final_dict:
                    for url in file_dict[date]:
                        if url in final_dict[date]:
                            final_dict[date][url] += file_dict[date][url]
                        else:
                            final_dict[date][url] = file_dict[date][url]
                else:
                    final_dict[date] = file_dict[date]
                
                if file_num == thread_count-1:
                    for url in final_dict[date]:
                        final_dict[date][url] = sum(final_dict[date][url]) / len(final_dict[date][url])
        
        # QUERY 6, PROBLEM 6.3.1
        sorted(final_dict["2022-09-03"].items(), key=lambda item: item[1])  # sort value ascending
        dates = ["2022-09-03", "2022-09-04", "2022-09-05"]
        for date in dates:
            print("\nDate       URL                         Average_TTFB")
            sorted_dict = sorted(final_dict[date].items(), key=lambda item: item[1])
            for TTFB in sorted_dict[:5]:  # print lowest 5 TTFBs' information
                print(f"{date} {TTFB[0]} {TTFB[1]}")
